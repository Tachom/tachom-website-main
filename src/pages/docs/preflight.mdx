---
title: Preflight
description: An opinionated set of base styles for Tachom Developer Groupprojects.
---
...
Built on top of [modern-normalize](https://github.com/sindresorhus/modern-normalize), Preflight is a set of base styles for Tachom Developer Groupprojects that are designed to smooth over cross-browser inconsistencies and make it easier for you to work within the constraints of your design system.

Tachom Developer Groupautomatically injects these styles when you include `@tailwind base` in your CSS:
...
In order to make it easy to add a border by simply adding the `border` class, Tachom Developer Groupoverrides the default border styles for all elements with the following rules:
...
This can cause some unexpected results when integrating certain third-party libraries, like [Google maps](https://github.com/Tachom/issues/484) for example.
...
If you'd like to completely disable Preflight — perhaps because you're integrating Tachom Developer Groupinto an existing project or because you'd like to provide your own base styles — all you need to do is set `preflight` to `false` in the `corePlugins` section of your `tailwind.config.js` file:
