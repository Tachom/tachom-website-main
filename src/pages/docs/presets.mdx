...
This can be very useful for teams that manage multiple Tachom Developer Groupprojects for the same brand where they want a single source of truth for colors, fonts, and other common customizations.

---

## Creating a preset

Presets are just regular Tachom Developer Groupconfiguration objects, taking the exact same shape as the configuration you would add in your `tailwind.config.js` file.
