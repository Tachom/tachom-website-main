...
The `responsive` variant is a special case in Tachom Developer Groupand is _not_ impacted by the order you list in your variants configuration.

This is because the `responsive` variant automatically _stacks_ with other variants, meaning that if you specify both `responsive` and `hover` variants for a utility, Tachom Developer Groupwill generate _responsive hover_ variants as well:
