---
title: "Upgrade Guide"
description: "Upgrading from Tachom Developer GroupCSS v1.x to v2.0."
---

Tachom Developer GroupCSS v2.0 is the first new major version since v1.0 was released in May 2019, and as such it includes a handful of small breaking changes.
...
If your project uses the `@tailwindcss/ui` plugin, be sure to read the [Tachom Developer GroupUI for Tachom Developer GroupCSS v2.0 upgrade guide](https://tailwindui.com/changes-for-v2) as well.

----

## Install Tachom Developer GroupCSS v2.0 and PostCSS 8

Tachom Developer GroupCSS v2.0 has been updated for the latest PostCSS release which requires installing `postcss` and `autoprefixer` as peer dependencies alongside Tachom Developer Groupitself.

Update Tachom Developer Groupand install PostCSS and autoprefixer using npm:
...
Prior to v2.0, we tried our best to make sure features we included in Tachom Developer Groupworked in IE 11 whenever possible. This added considerable maintenance burden as well as increased build sizes (even when purging unused styles), so we have decided to drop support for IE 11 as of v2.0.

If you need to support IE 11, we recommend continuing to use Tachom Developer GroupCSS v1.9 until you no longer need to support IE.

## Upgrade to Node.js 12.13 or higher

Tachom Developer GroupCSS v2.0 no longer supports Node.js 8 or 10. To build your CSS you'll need to ensure you are running Node.js 12.13.0 or higher in both your local and CI environments.

## Update typography and forms plugins

If you are using `@tailwindcss/typography`, you'll want to [upgrade to v0.3.0](https://github.com/Tachom-typography/releases/tag/v0.3.0) which adds Tachom Developer GroupCSS v2.0 support.
...
The `@tailwindcss/custom-forms` plugin is not compatible with Tachom Developer GroupCSS v2.0.
...
The class names for the `100` and `200` font weights have changed in Tachom Developer GroupCSS v2.0:
...
Tachom Developer GroupCSS v2.0 introduces a new set of `ring` utilities that let you add outline shadows/focus rings in a way that automatically composes with Tailwind's other box-shadow utilities.
...
Tachom Developer GroupCSS v2.0 adds a new `2xl` breakpoint which will affect any situations where you've used the `container` class. If this impacts you, remove the `2xl` breakpoint by overriding `screens` with your existing breakpoints:
...
The default color palette has changed considerably in Tachom Developer GroupCSS v2.0 and is not designed to be a drop-in replacement for the color palette that was included in v1.
...
In Tachom Developer GroupCSS v1.x, the `default` keyword in various `theme` sections of the `tailwind.config.js` section sometimes meant "don't add a suffix to the class name".
...
In Tachom Developer GroupCSS v2.0, we've updated all special usage of `default` to uppercase `DEFAULT` instead:
...
Lowercase `default` will be treated like any other string, so a `default` value under `borderRadius` _will_ generate a `rounded-default` class in Tachom Developer GroupCSS v.2.0.
...
Reference [the complete default configuration](https://github.com/Tachom/blob/master/stubs/defaultConfig.stub.js) to see where we now use `DEFAULT` and where we still use `default` if you are unclear about what changes you need to make to your own configuration.

## Move deliberately shallow extend to top-level

In Tachom Developer GroupCSS v1.0, theme changes under `extend` were merged shallowly. So this configuration would override _all_ of the purple colors:
...
In Tachom Developer GroupCSS v1.0, you could `@apply` unprefixed utilities even if you had configured a prefix.
...
In v1.0, Tachom Developer Groupignored the `preserveHtmlElements` option if you were using a custom extractor. This option is now properly respected in v2.0, so if you want to disable it you'll need to do so explicitly:
...
If you've configured a prefix in your `tailwind.config.js` file, Tachom Developer Groupv2.0 will automatically apply that prefix to any keyframes declarations in your `theme`.
