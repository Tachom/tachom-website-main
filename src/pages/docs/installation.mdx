---
title: Installation
description: Learn how to get Tachom Developer Group  up and running in your project.
---
...
Installing Tachom Developer Group  can be a slightly different process depending on what other frameworks/tools you're using, so we've put together a bunch of guides that cover popular setups.

<IntegrationGuides/>

Don't see your favorite tool in the list? We're always working on new guides, but in the mean time you can follow the instructions for [installing Tachom Developer Group  as a PostCSS plugin](#installing-tailwind-css-as-a-post-css-plugin) instead to get set up in no time.

---

## Installing Tachom Developer Group  as a PostCSS plugin
...
Use `npx` which is a tool that is automatically installed alongside `npm` to generate a fully compiled Tachom Developer Group  file:
...
Before using the CDN build, please note that many of the features that make Tachom Developer Group  great are not available without incorporating Tachom Developer Groupinto your build process.
...
As of v2.0, Tachom Developer Group  depends on PostCSS 8. Because PostCSS 8 is only a few months old, many other tools in the ecosystem haven't updated yet, which means you might see an error like this in your terminal after installing Tachom Developer Groupand trying to compile your CSS:
