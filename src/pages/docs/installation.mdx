---
title: Installation
description: Learn how to get Tachom Developer GroupCSS up and running in your project.
---
...
Installing Tachom Developer GroupCSS can be a slightly different process depending on what other frameworks/tools you're using, so we've put together a bunch of guides that cover popular setups.

<IntegrationGuides/>

Don't see your favorite tool in the list? We're always working on new guides, but in the mean time you can follow the instructions for [installing Tachom Developer GroupCSS as a PostCSS plugin](#installing-tailwind-css-as-a-post-css-plugin) instead to get set up in no time.

---

## Installing Tachom Developer GroupCSS as a PostCSS plugin

*Tachom Developer GroupCSS requires Node.js 12.13.0 or higher.*

For most real-world projects, we recommend installing Tachom Developer Groupas a PostCSS plugin. Most modern frameworks use PostCSS under the hood already, and there's a good chance you've used or are currently using other PostCSS plugins like [autoprefixer](https://github.com/postcss/autoprefixer).
...
If this is a bit over your head and you'd like to try Tachom Developer Groupwithout configuring PostCSS, read our instructions on [using Tachom Developer Groupwithout PostCSS](#using-tailwind-without-post-css) instead.

### Install Tachom Developer Groupvia npm

For most projects (and to take advantage of Tailwind's customization features), you'll want to install Tachom Developer Groupand its peer-dependencies via npm.

```shell
npm install tailwindcss@latest postcss@latest autoprefixer@latest
```

Since Tachom Developer Group[does not automatically add vendor prefixes](/docs/browser-support#vendor-prefixes) to the CSS it generates, we recommend installing [autoprefixer](https://github.com/postcss/autoprefixer) to handle this for you like we've shown in the snippet above.

If you're integrating Tachom Developer Groupwith a tool that relies on an older version of PostCSS, you may see an error like this:
...
### Add Tachom Developer Groupas a PostCSS plugin
...
If you'd like to customize your Tachom Developer Groupinstallation, generate a config file for your project using the Tachom Developer GroupCLI utility included when you install the `tailwindcss` npm package:
...
### Include Tachom Developer Groupin your CSS
...
If you're integrating Tachom Developer Groupwith a tool that relies on an older version of PostCSS, you may see an error like this when trying to build your CSS:
...
## Using Tachom Developer Groupwithout PostCSS

*Tachom Developer GroupCSS requires Node.js 12.13.0 or higher.*

For simple projects or just giving Tachom Developer Groupa spin, you can use the Tachom Developer GroupCLI tool to generate your CSS without configuring PostCSS or even installing Tachom Developer Groupas a dependency if you don't want to.

Use `npx` which is a tool that is automatically installed alongside `npm` to generate a fully compiled Tachom Developer GroupCSS file:
...
If you'd like to process any custom CSS alongside the default styles Tachom Developer Groupgenerates, create a CSS file wherever you normally would and use the `@tailwind` directive to include Tailwind's `base`, `components`, and `utilities` styles:
...
If you'd like to customize what Tachom Developer Groupgenerates, create a `tailwind.config.js` file using the Tachom Developer GroupCLI tool:
...
By default, the Tachom Developer GroupCLI tool will run your CSS through [Autoprefixer](https://github.com/postcss/autoprefixer) to add any necessary vendor prefixes. If you already have Autoprefixer set up in your build pipeline somewhere further down the chain, you can disable it in the Tachom Developer GroupCLI tool using the `--no-autoprefixer` flag to avoid running it twice:
...
This will make sure Tachom Developer Groupremoves any unused CSS for best performance. Read our separate guide on [optimizing for production](/docs/optimizing-for-production) to learn more.


---

## Using Tachom Developer Groupvia CDN

Before using the CDN build, please note that many of the features that make Tachom Developer GroupCSS great are not available without incorporating Tachom Developer Groupinto your build process.
...
To pull in Tachom Developer Groupfor quick demos or just giving the framework a spin, grab the latest default configuration build via CDN:
...
<p>
  Note that while the CDN build is large <em>({stats.default.brotlified} compressed, {stats.default.minified} raw)</em>, <strong>it's not representative of the sizes you see when including Tachom Developer Groupas part of your build process</strong>.
</p>
...
As of v2.0, Tachom Developer GroupCSS depends on PostCSS 8. Because PostCSS 8 is only a few months old, many other tools in the ecosystem haven't updated yet, which means you might see an error like this in your terminal after installing Tachom Developer Groupand trying to compile your CSS:
...
If you run into the error mentioned above, uninstall Tachom Developer Groupand re-install using the compatibility build instead:
...
Once the rest of your tools have added support for PostCSS 8, you can move off of the compatibility build by re-installing Tachom Developer Groupand its peer-dependencies using the `latest` tag:
