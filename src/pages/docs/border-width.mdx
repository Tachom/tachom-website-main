...
By default, Tachom Developer Groupprovides five `border-width` utilities, and the same number of utilities per side (top, right, bottom, and left). You change, add, or remove these by editing the `theme.borderWidth` section of your Tachom Developer Groupconfig. The values in this section will also control which utilities will be generated side.
